...
import { hasCommentMarker } from './print';
...
export function collectImportDeclarations(ast: AnyNode): AnyNode[] {
...
export function normalizeImportDeclaration(node: AnyNode, text: string): NormalizedImport | null {
  if (hasImportAttributes(node) || hasCommentMarker(text.slice(node.start, node.end))) {
    return null;
  }
...
function hasImportAttributes(node: AnyNode): boolean {
  return Boolean(
...
function printImportSpecifier(specifier: AnyNode, declarationIsTypeOnly: boolean): string {
...
function getImportedName(specifier: AnyNode): string {
...
function getLocalName(specifier: AnyNode): string {
  return specifier.local?.name ?? '';
}

function getSourceRaw(node: AnyNode): string {
